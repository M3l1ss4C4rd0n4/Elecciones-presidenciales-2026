...
<section id="comparativo" class="section">
  ...
  <div class="stats-grid" style="margin-bottom:16px">
    <div class="stat-card">
      ...
      <div class="stat-num" id="comp-mejor">-</div>
    ...
    <div class="stat-card">
      ...
      <div class="stat-num" id="comp-peor">-</div>
    ...
    <div class="stat-card">
      ...
      <div class="stat-num" id="comp-pct22">-</div>
    ...
    <div class="stat-card">
      ...
      <div class="stat-num" id="comp-pct26">-</div>
  ...
  <div class="chart-row">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="card-subtitle">Variacion en puntos porcentuales entre elecciones</div>
      </div>
      <div class="card-body"><div class="chart-box" id="chart-comp-box"></div></div>
    </div>
  </div>
...
function renderComp() {{
  ...
  document.getElementById('comp-pct22').textContent = data.reduce(function(s, r) {{ return s + r.pct22; }}, 0) / data.length + '%';
  document.getElementById('comp-pct26').textContent = data.reduce(function(s, r) {{ return s + r.pct26; }}, 0) / data.length + '%';
}}
...
// Chart 8: Box-style distribution of deltas per pair (using scatter/bar)
(function() {{
  ...
  chart.setOption({{
    tooltip: {{ trigger:'axis', formatter: function(ps) {{
      var d = ps[0].data;
      return '<strong>' + ps[0].seriesName + '</strong><br>Min: ' + d[1].toFixed(1) + ' pp<br>Q1: ' + d[2].toFixed(1) + ' pp<br>Med: ' + d[3].toFixed(1) + ' pp<br>Q3: ' + d[4].toFixed(1) + ' pp<br>Max: ' + d[5].toFixed(1) + ' pp';
    }} }},
    grid: {{ left:60, right:30, top:30, bottom:60 }},
    xAxis: {{ type:'category', data: compPairs, axisLabel:{{ fontSize:9, width:70, overflow:'break' }} }},
    yAxis: {{ type:'value', name:'Delta (pp)', splitLine:{{ lineStyle:{{ color:'#f0f0f0' }} }} }},
    series: series
  }});
  window.addEventListener('resize', function() {{ chart.resize(); }});
}})();
