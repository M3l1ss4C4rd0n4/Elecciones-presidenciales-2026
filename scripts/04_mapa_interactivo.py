  ...
  function buildPuestosLayer(data) {{
    ...
    L.geoJSON(data, {{
      ...
      onEachFeature: function(feature, layer) {{
        var p = feature.properties;
        layer.bindTooltip('<div class="tt-name">' + (p.NOMBRE || 'Puesto') + '</div>' +
          '<div class="tt-sub">Ganador: ' + p.GANADOR + '</div>' +
          '<div class="tt-val">' + Number(p.TOTAL_VOTOS).toLocaleString() + ' votos</div>');
        var popupContent = '<div class="pp"><div class="pp-header"><span class="pp-title">' +
          (p.NOMBRE || 'Puesto') + '</span><span class="pp-subtitle">' +
          (p.DIRECCION || '') + '</span></div></div>';
