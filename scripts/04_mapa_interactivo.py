...
<style>
  ...
  .puesto-tooltip {{ font-size:11px !important; padding:6px 10px !important; line-height:1.4 !important; }}
  .puesto-tooltip b {{ font-size:12px; }}
...
(function() {{
  ...
  // ─── PUESTOS LAYER ───
  function setPuestosMode(on) {{
    // Toggle polygon interactivity so clicks reach circle markers
    [upzLayer, locLayer].forEach(function(layer) {{
      layer.eachLayer(function(l) {{
        if (l._path) l._path.style.pointerEvents = on ? 'none' : '';
        if (on) {{
          l.setStyle({{ fillOpacity:0.08, weight:0.3, color:'#ccc' }});
        }} else {{
          applyStyle(l);
        }}
      }});
  ...
  window.togglePuestos = function() {{
    ...
    if (checked) {{
      ...
      updateMap(); // restore polygon styles
    }}
  }};
  ...
  function buildPuestosLayer(data) {{
    ...
    L.geoJSON(data, {{
      pointToLayer: function(feature, latlng) {{
        ...
        var r = Math.max(6, Math.sqrt(p.TOTAL_VOTOS || 200) / 8);
        var color = candColors[p.GANADOR] || '#ccc';
        var m = L.circleMarker(latlng, {{
          radius: r, fillColor: color,
          color: 'white', weight: 1, fillOpacity: 0.85
        }});
        // Tooltip on mouseover via label (more reliable than Leaflet tooltip with clusters)
        var nom = p.NOMBRE || 'Puesto';
        var gan = p.GANADOR || 'N/A';
        var total = Number(p.TOTAL_VOTOS || 0);
        var pct = Number(p.PCT_GANADOR || 0);
        var loc = p.LOCALIDAD || '';
        m.bindTooltip('<b>' + nom + '</b><br>' + loc + '<br>' + gan + ': ' + total.toLocaleString() + ' v (' + pct.toFixed(1) + '%)', {{
          sticky:true, direction:'top', offset:L.point(0,-4), className:'puesto-tooltip'
        }});
        return m;
      }},
      onEachFeature: function(feature, layer) {{
        ...
        layer.bindPopup(
          '<div class="pp"><div class="pp-header"><span class="pp-title">' + nom +
  ...
  document.querySelectorAll('.toggle-btn').forEach(function(btn) {{
    btn.addEventListener('click', function() {{
      ...
      if (document.getElementById('toggle-puestos').checked) setPuestosMode(true);
      updateMap();
    }});
  }});
